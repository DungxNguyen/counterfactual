...
class CalibNNThreeOutputs(nn.Module):
    ...
    def forward(self, x, meta):
        ...
        out3 = self.sigmoid(self.out_layer3(emb_mean).reshape((CONFIGS[self.params['disease']]["num_patch"], CONFIGS[self.params['disease']]["num_patch"])))*0.2 - 0.1
...
def build_simulator(params,devices,seed_infection_status):
    ...
    if 'meta' in params['model_name']:
        ...
        abm = MetapopulationSEIRM(params, devices[0], 10, torch.Tensor(ma_flow_data.values), torch.Tensor(population_data['population'].values), seed_infection_status)
    ...
    if 'bogota' in params['disease']:
        ...
        abm = MetapopulationSEIRM(params, devices[0], CONFIGS[params['disease']]["num_patch"], torch.Tensor(flow_data.values), torch.Tensor(population_data['Population'].values), seed_infection_status)
...
def runner(params, devices, verbose, args, seed_infection_status):
    for run_id in range(params['num_runs']):
        ...
        if params['disease']=='COVID':
            ...
            train_loader, metas_train_dim, X_train_dim, seqlen = \
                fetch_age_group_data_covid(batch_size=batch_size,noise_level=params['noise_level'], args=args, split="train_smaller")

            
            test_loader, _, _, _ = \
                fetch_age_group_data_covid(batch_size=batch_size,noise_level=params['noise_level'], args=args, split="test_smaller")
        ...
        if train_flag:
            ...
            for epi in range(num_epochs):
                ...
                for batch, (x, y) in enumerate(train_loader):   
                    abm = build_simulator(copy(params),devices, seed_infection_status)
        ...
        with torch.no_grad():
            for batch, (x, y) in enumerate(test_loader):
                ...
                abm = build_simulator(params,devices, seed_infection_status)
                # forward pass param model
                meta = torch.Tensor([[1]])
                counties = torch.ones(16)
                meta = meta.to(devices[0])
